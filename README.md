# microsites

Small self-contained sites, one folder per subdomain of [arshnah.in](https://arshnah.in).
Each is a single `index.html`, no build step, deployed as its own Vercel project.

| folder | subdomain | what it is |
|---|---|---|
| `now/` | now.arshnah.in | a live `/now` page — Discord + Spotify status via Lanyard, last commit via GitHub, all client-side |
| `card/` | card.arshnah.in | a Certified LARPer&trade; ID-card generator (canvas → PNG) |
| `shame/` | shame.arshnah.in | the Hall of Shame — a museum of every injection that did nothing |

Companion to [larp.arshnah.in](https://larp.arshnah.in) and the [larpring](https://larpring.github.io).
